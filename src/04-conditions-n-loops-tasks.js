...
function getCommonDirectoryPath(pathes) {
  ...
  const splitPaths = pathes.map(function pathSplitter(path) {
    return path.split('/');
  });
  ...
  for (let i = 0; i < splitPaths[0].length; i += 1) {
    ...
    if (
      splitPaths.every(function pathChecker(arr) {
        return arr[i] === segment;
      })
    ) {
  ...
  return (
    commonParts.join('/') +
    (commonParts[commonParts.length - 1] === '' ? '' : '/')
  );
}
...
function evaluateTicTacToePosition(position) {
  ...
  lines.forEach(([a, b, c]) => {
    const val = position[a[0]]?.[a[1]];
    if (
      val &&
      val === position[b[0]]?.[b[1]] &&
      val === position[c[0]]?.[c[1]]
    ) {
      winner = val;
    }
  });
